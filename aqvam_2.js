...
export default function(){
  ...
  const mTentacle = new THREE.MeshPhysicalMaterial({
    color: 0xffffff,    
  });
  ...
  for(let i=0; i<200; i++){  
    ...
    cellBody.position.set( 100, 0, 0 );
    cellBody.scale.set( 1, 1, 0.5 );
    
    scene.add(cellBody);

    const gBodyI = new THREE.SphereGeometry( 10, 20, 20 );  
    const mBodyI = new THREE.MeshPhysicalMaterial({
      color: 0xffffff,     
      opacity: 0,
    });

    const cellClone = new THREE.Mesh(gBodyI, mBodyI);  
    scene.add(cellClone);
    cellClone.position.set( 0, 0, 0 );

    let cellRotation = new THREE.Group();
    cellRotation.add(cellBody, cellClone);
    cellRotation.rotation.set(0,0,0);
    scene.add(cellRotation);    
    
    let batterioClone = cellRotation.clone();
    batterioClone.rotation.set(0,i/33, 0);
    batterioClone.position.set(0,0, 0);  
